...
* 增量重建和文件监控等功能不受影响
* 配置文件完全向后兼容，新配置项使用默认值
[2025-06-21 14:18:00] - **IndexingTaskManager 嵌入模型并发调用架构决策**

## 决策

实施全面的嵌入模型并发调用改进，采用多层并发架构设计，支持文件级并发、嵌入向量级并发和配置驱动的并发控制，全面覆盖批处理索引、增量重建和单文件更新三个核心场景。

## 理由

1. **性能瓶颈急需解决**：当前串行处理导致大型代码库索引时间过长，资源利用率低（CPU 20-30%）
2. **用户体验需求**：用户明确要求"全面改进所有嵌入向量调用场景"的并发性能
3. **技术架构成熟度**：现有嵌入向量抽象层架构完善，支持无缝集成并发机制
4. **硬件资源未充分利用**：多核 CPU 和网络带宽资源存在大量浪费

## 实现细节

* **核心架构组件**：
  - ConcurrentEmbeddingManager：核心并发调度和资源管理
  - ConcurrencySettings：配置驱动的并发参数控制
  - IntelligentBatchSplitter：智能批次分割和负载均衡
  - 完善的错误处理和重试机制

* **并发策略分层**：
  - 文件级并发：多个文件批次同时处理 (MaxConcurrentFileBatches: 2-4)
  - 嵌入向量级并发：单个提供商多请求并发 (MaxConcurrentEmbeddingRequests: 4-8)
  - 智能批次分割：大批次拆分为多个小批次并发处理

* **三大场景全覆盖**：
  - ProcessCodebaseInBatchesAsync：批处理索引并发优化
  - ExecuteIncrementalRebuildAsync：增量重建并发处理
  - UpdateFileIndexAsync：单文件更新并发改进

* **技术保障机制**：
  - SemaphoreSlim 并发度控制和资源保护
  - 指数退避重试和失败回退机制
  - 批次级错误隔离，单个失败不影响整体进度
  - 完整的向后兼容性保障
...
* **性能显著提升**：索引时间减少 50-70%，CPU 利用率提升至 60-80%
* **内存使用优化**：从 O(n) 降低到 O(batch_size)，减少 70-80% 内存占用
* **用户体验改善**：更精确的进度反馈，更快的索引完成时间
* **系统扩展性增强**：支持更大规模代码库的高效处理

## 技术风险和缓解

* **内存压力风险**：通过智能批次大小控制和及时资源释放缓解
* **网络限流风险**：配置化并发度设置和指数退避重试机制缓解
* **错误传播风险**：批次级错误隔离和失败回退机制缓解
* **兼容性风险**：保留原有串行方法作为备选，渐进式启用并发功能
...
- 阶段四：配置集成和测试优化（1小时）
[2025-06-21 15:16:00] - **CodebaseApp 全新升级架构决策**

## 决策

设计并实施 CodebaseApp 的全面架构升级，从现有基于JSON文件存储的MCP服务器升级为具备Web管理界面、SQLite数据库存储、领域驱动设计的企业级代码索引平台。

## 理由

1. **功能局限性突破需求**：现有系统无法满足灵活的文件类型配置、可视化管理、实时监控等高级需求
2. **架构现代化需求**：JSON文件存储已成为扩展瓶颈，需要事务性数据库支持复杂的并发场景
3. **用户体验提升需求**：命令行操作和手动配置编辑不适应现代化管理需求
4. **系统可维护性改进**：当前单一服务架构耦合度高，需要清晰的领域分离
...
- **前端技术栈**：HTML5 + Bootstrap 5 + Chart.js (轻量级、快速开发)
- **数据库选择**：SQLite (零配置、高性能、易部署)
- **实时通信**：SignalR Core (原生.NET支持)
- **部署方式**：Docker化支持 + 传统部署兼容

### 实施策略：

- **渐进式升级**：6个阶段分步实施，每阶段可独立验证
- **向后兼容**：保证现有MCP客户端无需修改
- **风险控制**：完整备份机制和回滚策略
- **时间预期**：13-17天完成全部升级
...
- **配置灵活性**：支持多种文件类型和项目预设
- **管理效率**：Web界面操作效率提升5-10倍
- **系统稳定性**：数据库事务保证一致性，故障恢复能力增强
- **扩展能力**：为AI代码分析、团队协作等高级功能奠定基础

### 风险评估：

- **高风险**：数据迁移过程，通过完整备份和分步验证缓解
- **中等风险**：性能回归可能，通过基准测试和监控缓解
- **低风险**：学习成本，通过渐进式发布和文档支持缓解

此决策将使 CodebaseApp 从工具级产品升级为平台级解决方案，显著提升用户体验和系统能力。
- 总计：6-8小时完成全面改进